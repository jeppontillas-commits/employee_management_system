...
<div class="container-fluid">
    <h2 class="mb-4"><i class="fas fa-plus-circle"></i> Create System Setting</h2>
                    ...
                    <form action="{{ route('system-settings.store') }}" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('setting_type') is-invalid @enderror" 
                                   id="setting_type" name="setting_type" value="{{ old('setting_type') }}" required>
                            <small class="form-text text-muted">e.g., app_name, max_employees, etc.</small>
                            @error('setting_type')
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control @error('setting_value') is-invalid @enderror" 
                                      id="setting_value" name="setting_value" rows="4">{{ old('setting_value') }}</textarea>
                            @error('setting_value')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-select @error('modified_by') is-invalid @enderror" id="modified_by" name="modified_by">
                                ...
                                @foreach($users as $user)
                                    <option value="{{ $user->user_id }}" {{ old('modified_by') == $user->user_id ? 'selected' : '' }}>
                                        {{ $user->user_name }}
                                    </option>
                                @endforeach
                            </select>

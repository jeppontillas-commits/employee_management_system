...
@section('title', 'Create Deletion Log')

@section('content')
<div class="container-fluid">
    <h2 class="mb-4"><i class="fas fa-plus-circle"></i> Create Deletion Log</h2>
        ...
        <div class="col-md-8">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('deletion-logs.store') }}" method="POST">
                        @csrf
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control @error('dependency') is-invalid @enderror" 
                                      id="dependency" name="dependency" rows="4" placeholder="List any dependencies or related records">{{ old('dependency') }}</textarea>
                            @error('dependency')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-select @error('validated_by') is-invalid @enderror" id="validated_by" name="validated_by">
                                ...
                                @foreach($users as $user)
                                    <option value="{{ $user->user_id }}" {{ old('validated_by') == $user->user_id ? 'selected' : '' }}>
                                        {{ $user->user_name }}
                                    </option>
                                @endforeach
                            </select>
